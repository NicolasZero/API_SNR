...
-- Crea las tablas generales (datos privados que necesitan varios módulos)
CREATE TABLE general.persons(
    id integer NOT NULL UNIQUE GENERATED ALWAYS AS IDENTITY (START WITH 1),
    ...
    ethnicity_id integer NOT NULL DEFAULT 1,
...
-- Crea las tablas de auteorización (datos privados de los usuarios y permisos)
...
-- Crea las relaciones de las 
-- general persons
ALTER TABLE general.persons ADD CONSTRAINT fk_persons_gender_id FOREIGN KEY (gender_id) references genders(id);

ALTER TABLE general.persons ADD CONSTRAINT fk_persons_civil_status_id FOREIGN KEY (civil_status_id) references civil_status(id);

ALTER TABLE general.persons ADD CONSTRAINT fk_persons_ethnicity_id FOREIGN KEY (ethnicity_id) references ethnicity(id);

-- general location
ALTER TABLE general.location ADD CONSTRAINT fk_location_person_id FOREIGN KEY (person_id) references general.persons(id);
...
-- municipio
ALTER TABLE municipalities ADD CONSTRAINT fk_municipalities_state_id FOREIGN KEY (state_id) references states(id);

-- parroquia
ALTER TABLE parishes ADD CONSTRAINT fk_parishes_municipality_id FOREIGN KEY (municipality_id) references municipalities(id);

-- auth users
ALTER TABLE auth.users ADD CONSTRAINT fk_users_rol_id FOREIGN KEY (role_id) references auth.roles(id);

ALTER TABLE auth.users ADD CONSTRAINT fk_users_department_id FOREIGN KEY (department_id) references auth.departments(id);

ALTER TABLE auth.users ADD CONSTRAINT fk_users_gender_id FOREIGN KEY (gender_id) references genders(id);

-- auth location
ALTER TABLE auth.location ADD CONSTRAINT fk_auth_location_user_id FOREIGN KEY (user_id) references auth.users(id);

ALTER TABLE auth.location ADD CONSTRAINT fk_auth_location_state_id FOREIGN KEY (state_id) references states(id);

ALTER TABLE auth.location ADD CONSTRAINT fk_auth_location_municipality_id FOREIGN KEY (municipality_id) references municipalities(id);

ALTER TABLE auth.location ADD CONSTRAINT fk_auth_location_parish_id FOREIGN KEY (parish_id) references parishes(id);


-- Crea las Vistas que se usaran
-- auth location
CREATE VIEW view_user_location AS SELECT l.*, s.state, m.municipality, p.parish FROM auth.location AS l
...
-- auth user
CREATE VIEW view_user_data AS SELECT u.*, r.role, d.department, g.gender, l.state_id, l.state, l.municipality_id, l.municipality, l.parish_id, l.parish, l.address FROM auth.users AS u
...
LEFT JOIN view_user_location AS l ON u.id = l.user_id;

-- general location
CREATE VIEW view_person_location AS SELECT l.*, s.state, m.municipality, p.parish FROM general.location AS l
LEFT JOIN states AS s ON l.state_id = s.id
LEFT JOIN municipalities AS m ON l.municipality_id = m.id 
LEFT JOIN parishes AS p ON l.parish_id = p.id;
...
INSERT INTO civil_status (civil_status) VALUES ('Soltero/a'),('Casado/a'),('Divorciado/a');
INSERT INTO ethnicity (ethnicity) VALUES ('Moreno/a'),('Blanco/a'),('Negro/a'),('Afrodescendiente'),('indigena'),('Otro');
